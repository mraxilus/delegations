...
type
  ...
  Marker* = object ## Define one object's marker, ready to draw on foreground layer.
    ...
    label_at*: ScreenPosition ## Where name label is centred, in screen space.
      ...
      ## stands inside its bottom-left corner instead, `MARGIN_LABEL_FOOT` up.
      ## Rails and frame: anchor *on* outline instead, with label pushed off it; see
      ## `is_label_beside`.
    ...
    is_label_beside*: bool ## Whether `label_at` is anchor on outline, label pushed off it.
      ...
      ## its own text box. Frame: `label_at` lies on frame's left edge, pushed rightward
      ## same way, so text starts clear of edge whatever its width. False elsewhere:
      ## `label_at` is centre itself.
...
func markerFrame(width, height: int; progress, clearance: float; marker: var Marker): bool =
  ...
  while next_corner < CORNERS_MARKER_FRAME:
    ...
    inc next_corner
  # Inside bottom-left corner: frame is whole view, and above it is off screen. Anchor on
  #   frame's left edge, `MARGIN_LABEL_FOOT` up, pushed rightward by front-end's own
  #   measured text as line's label is pushed off its rail; see `is_label_beside`.
  #   Not inside top edge, centred, where it stood under chip row's controls.
  marker.has_label = true
  marker.is_label_beside = true
