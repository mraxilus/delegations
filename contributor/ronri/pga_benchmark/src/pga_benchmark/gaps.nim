...
##   `TIME_BAND`, since medians on shared machine wander. Identifiers come from ledger,
...
type
  ...
  Row* = object
    ...
    key*: string
      ## `<config>/<probe id>`, ledger key.
    id*: string
      ## Ledger identifier, e.g. `G017`.
    config*, probe*: string
  ...
  Ledger* = object
    ## Define identifier ledger: next number and every key allotted so far.
    next*: int
...
func ledgerOf*(node: JsonNode): Ledger =
  ## Read ledger from its document; empty ledger where document is nil.
  result.next = 1
...
func toJson*(l: Ledger): JsonNode =
  ## Shape ledger as document, keys sorted so file moves only where ids do.
  var ids = newJObject()
  for key in toSeq(l.ids.keys).sorted: ids[key] = %l.ids[key]
  %*{"schema": SCHEMA, "kind": KIND_LEDGER, "next": l.next, "ids": ids}


func assign*(rows: var seq[Row]; ledger: var Ledger) =
  ## Give every row its identifier, allotting next number to keys ledger lacks.
  for row in rows.mitems:
    if row.key notin ledger.ids:
      ledger.ids[row.key] = "G" & align($ledger.next, 3, '0')
      inc ledger.next
    row.id = ledger.ids[row.key]
...
func render*(
  ...
  lines.add wrap(
    ...
    "hand. Identifiers are stable: `baseline/ledger.json` maps every row to its number and " &
...
func generate*(algebras: openArray[Algebra]; ledger: Ledger): (string, Ledger) =
  ## Generate list and grown ledger from documents.
  var rows: seq[Row]
