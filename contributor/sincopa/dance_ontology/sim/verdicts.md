...
answers in its own physical words, and `sim/words.nim` translates once.  The whole-cloth page reads
that same table, so the page and this report cannot give one pose two answers:

| the sim says | the sheet says |
|---|---|
| the arm out in front, on neither face of its own body | open |
