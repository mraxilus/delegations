...
##     which.
##   Controls are dance's freedoms and none of rig's: which
##     hands are held, at which level, and one quarter turn per move for
##     either dancer -- on their own axis, or in orbit round their partner,
##     where walker keeps facing centre and so turns as far as they
##     travel (project's rule 32).  Bodies are average adult's
##     and do not change.  How far apart couple stand is not control
##     either: they stand, and go on standing as they turn, wherever
##     joints are furthest from their limits -- stepping in or out
##     one centimetre each time whenever that leaves arms freer.
##   Quarter is walked in small moves, as sweeps walk turn, so
##     wound arm stays wound; where no small move holds whole quarter
##     is refused -- couple are put back where they started it, and
##     page says how far it got and what blocked it -- so tallies are
##     always whole quarters.  Couple never stand closer than ten
##     centimetres of clear air between their torsos, whichever way they
##     face.  Pose is never settled afresh mid-turn -- fresh pose
##     has no memory, and would lay arm through body as happily as
##     round it -- except one re-organisation sweep allows too, to
##     pose that goes round bodies same way.  Changing hands or
##     level settles pose afresh, face to face.
##   Levels carry sheet's names -- low, high, above -- which is
##     one translation this page makes, as `sim/verdicts` makes it;
##     model underneath knows only torso, neck and crown.
